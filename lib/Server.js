...
class Server {
	...
	async retrieve(req, res) {
		...
		const columns = { player: 't1.player', value: 't1.value', created: 't1.created_at' };
		let queries = contexts.value.map(ctx => filterObj.filter(
			this.db('score as t1')
			.select(columns)
			.leftJoin('score as t2', (that) => {
				that.on('t1.player','=','t2.player').andOn('t1.value ','>',' t2.value')
			})
			.where('t1.context', ctx.id)
			.where('t2.id', null)
		));

		// console.log(queries[0].toSQL().toNative())
		...
		try {
			this.db.insert(score.value)
				...
				.then(function (result) {
					let tmp = score.value;
					tmp.id = result[0];
					res.json({ success: true, message: 'ok', result: tmp });     // respond back to request
				})
				.catch(function (err) {
	...
	async constructScore(req, res) {
		...
		const score = this.evaluate(body, context.value, player.value);
		if (score.error) {
	...
	saveScore(scoreObj) {
		return this.db.insert(scoreObj.value).into('score');
	}
